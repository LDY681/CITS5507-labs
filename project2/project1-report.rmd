...
# Introduction
Matrix multiplication becomes increasingly expensive as the dimension size grows, particularly with sparse matrices, which contain mostly zero values and only a few non-zero elements. This results in unnecessary computations involving zeros, making the process inefficient in terms of both time and memory. In this project, we implemented a customized, simplified version of compressed matrix multiplication to tackle these inefficiencies. In project 1, we have explored how OpenMP helps to parallelize and speed up the compressed matrix multiplication with various threads count and parallelization strategies. In the project 2, we would like to integrate MPI on top of OpenMP, and conduct a a series of experiments with different approaches(Sequential, Pure OpenMP, Pure MPI and MPI+OpenMP) and different combinations of parameters(Number of nodes, Number of processes per node and Number of threads per threads). Due to usage shortage on Pawsey machine, we have changed our objective to evaluate which settings can complete task more efficiently rather than discovering which can compute the most matrix size within 10 minutes given time.
...
In the second step, we conducted experiments using different node counts, process counts and thread counts. We monitored the performance using Sequential/MPI/OpenMP/MPI+OpenMP parallelization on Pawsey supercomputer. Performance was evaluated based on execution time across various settings. To save computing powers and tackle with the usage shortage situation, our experiments were only conducted on several signature cases rather than do a bulk action manner.

\vspace{0.5cm}

# Implementation
## Matrix Generation (KEEP)
...
The function `compressedMatrixMultiply()` leverages both MPI and OpenMP to parallelize the multiplication of compressed matrices. It operates by splitting the the total for-loop into several sup-loops per process each with a range of `start_row` and `end_row`, then it iterating over the compressed representation, and the sub-loop is then parallelized using OpenMP's `parallel for` directive, which distributes the work among a given number of threads.
...
Finally, the generated matrices can be saved into local files for future use, with a suffix added to distinguish between different matrices. For example, in `FileB_matrixX_percent_1`, "FileB" indicates that it’s the value matrix, "matrixX" refers to the first matrix in the multiplication, and "percent_1" denotes that the matrix was generated with a 1% density of non-zero elements.

## MPI and OpenMP flags (TODO)
...
Process 0 handles the generation of compressed matrices, after compelte generation, the resulting matrices are flattened into 1-d array for MPI broadcasting, all process would receive the array to make sure they are working on the same data. 

 A MPI barrier is applied at the end to make sure all processes has done the computation and use `MPI_Send` to send the result, the process 0 will use `MPI_Recv` to gather the results and 
